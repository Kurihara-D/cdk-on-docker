    ...
    constructor(
      ...
      createResources(scope: Construct) {
        ...
        this.appContainerDef = new ContainerDefinition(scope, "appContainer", {
          containerName: `app-${envType}-container`,
          taskDefinition: this.taskDefinition,
    
          image: ContainerImage.fromEcrRepository(
            Repository.fromRepositoryName(scope, "appImage", `app-${envType}`)
          ),
        ...
        this.nginxContainerDef = new ContainerDefinition(scope, "nginxContainer", {
          containerName: `nginx-${envType}-container`,
          taskDefinition: this.taskDefinition,
    
          image: ContainerImage.fromEcrRepository(
            Repository.fromRepositoryName(scope, "nginxImage", `nginx-${envType}`)
          ),
          logging: LogDriver.awsLogs({
            streamPrefix: envType,
            logGroup: this.logGrp,
        ...
        this.nginxContainerDef.addVolumesFrom({
          sourceContainer: `app-${envType}-container`,
          readOnly: false,
        });
